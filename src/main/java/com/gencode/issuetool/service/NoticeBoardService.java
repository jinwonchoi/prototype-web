...
public class NoticeBoardService {
	...
	public Optional<NoticeBoardEx> addPostEx(NoticeBoard t) {
		long noticeId = noticeBoardDao.register(t);
		noticePersonalDao.insertAll(noticeId, Constant.PUSH_TAG_PROTOTYPE_MY_NOTICE_NOTICE.get());
	...
	public Optional<NoticeBoardWithFileList> loadPostEx(NoticePersonal noticePersonal) {
		noticePersonal.setNoticeType(Constant.PUSH_TAG_PROTOTYPE_MY_NOTICE_NOTICE.get());
		noticeBoardDao.incReadCnt(noticePersonal.getRefId());
		if (noticePersonal.getUserId() > 0) {
			noticePersonalDao.forceDelete(noticePersonal);
		}
		pushService.sendMsg("all", Constant.PUSH_TAG_PROTOTYPE_NOTICE_BOARD_INC_CNT.get(), Long.toString(noticePersonal.getRefId()));
		NoticeBoardWithFileList result = new NoticeBoardWithFileList(noticeBoardDao.loadEx(noticePersonal.getRefId()).get());
		List<FileInfo> attachedFileList = fileInfoDao.getFilesByRefId(noticePersonal.getRefId(), Constant.FILE_REFERENCE_REF_TYPE_ADDFILE.get()).get();
		List<FileInfo> embededFileList = fileInfoDao.getFilesByRefId(noticePersonal.getRefId(), Constant.FILE_REFERENCE_REF_TYPE_NORMAL.get()).get();
	...
	public void deletePost(long id) {
		...
		noticePersonalDao.deleteByRefId(id, Constant.PUSH_TAG_PROTOTYPE_MY_NOTICE_NOTICE.get());
	...
	public Optional<List<NoticePersonalEx>> searchMyNotice(Map<String, String> map) {
		return noticePersonalDao.searchEx(map);
	}
	...
	public void incReadCnt(long noticeId, long userId) {
		...
		if (userId > 0) {
			noticePersonalDao.forceDelete(new NoticePersonal(noticeId, userId, Constant.PUSH_TAG_PROTOTYPE_MY_NOTICE_NOTICE.get()));
		}
		pushService.sendMsg("all", Constant.PUSH_TAG_PROTOTYPE_NOTICE_BOARD_INC_CNT.get(), Long.toString(noticeId));
	}
