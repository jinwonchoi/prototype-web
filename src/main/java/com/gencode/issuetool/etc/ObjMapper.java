...
public class ObjMapper {
	...
	static public ChatInfo toChatInfo(ResultSet resultSet) throws SQLException {
		return toChatInfo("", resultSet);
	}
	static public ChatInfo toChatInfo(String prefix, ResultSet resultSet) throws SQLException {
		
		ChatInfo obj = new ChatInfo();
		if (resultSet.getLong(prefix+"ID")==0) return null;
		obj.setId(resultSet.getLong(prefix+"ID"));
		obj.setSenderId(resultSet.getLong(prefix+"SENDER_ID"));
		obj.setReceiverId(resultSet.getLong(prefix+"RECEIVER_ID"));
		obj.setLastMessageId(resultSet.getLong(prefix+"LAST_MESSAGE_ID"));
		obj.setLastMessage(resultSet.getString(prefix+"LAST_MESSAGE"));
		obj.setUnreadCnt(resultSet.getLong(prefix+"UNREAD_CNT"));
		obj.setStatus(resultSet.getString(prefix+"STATUS"));
		obj.setUpdatedDtm(Utils.DtToStr(resultSet.getTimestamp(prefix+"UPDATED_DTM")));
		obj.setCreatedDtm(Utils.DtToStr(resultSet.getTimestamp(prefix+"CREATED_DTM")));
	...
	static public MessageLog toMessageLog(String prefix, ResultSet resultSet) throws SQLException {
		
		MessageLog obj = new MessageLog();
		if (resultSet.getLong(prefix+"ID")==0) return null;
		obj.setId(resultSet.getLong(prefix+"ID"));
		obj.setChatId(resultSet.getLong(prefix+"CHAT_ID"));
		obj.setSenderId(resultSet.getLong(prefix+"SENDER_ID"));
		obj.setReceiverId(resultSet.getLong(prefix+"RECEIVER_ID"));
		obj.setStatus(resultSet.getString(prefix+"STATUS"));
		obj.setMessage(resultSet.getString(prefix+"MESSAGE"));
		obj.setUpdatedDtm(Utils.DtToStr(resultSet.getTimestamp(prefix+"UPDATED_DTM")));
		obj.setCreatedDtm(Utils.DtToStr(resultSet.getTimestamp(prefix+"CREATED_DTM")));
