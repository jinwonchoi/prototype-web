...
public class ChatController {
    ...
    public ResultObj<ChatInfo> openChat(@RequestBody ChatInfo t){
        try {
        	Optional<ChatInfo> chatInfo = chatService.openChat(t);
			if (chatInfo.isPresent()) {
				return new ResultObj<ChatInfo>(ReturnCode.SUCCESS, chatInfo.get());
			} else {
				return ResultObj.<ChatInfo>dataNotFound();
			}
        } catch (Exception e) {
        	logger.error("normal error", e);
			return ResultObj.<ChatInfo>errorUnknown();
        }
	}    
...
//	}
     ...
     */    
	@RequestMapping(method=RequestMethod.POST, value="/done/{id}")
