...
public class NoticeBoardController {
	...
	public ResultObj<List<NoticePersonalEx>> searchMyNotice(@RequestBody Map<String, String> map) {
		try {
			Optional<List<NoticePersonalEx>> list = noticeBoardService.searchMyNotice(map);
			if (list.isPresent()) {
				return new ResultObj<List<NoticePersonalEx>>(ReturnCode.SUCCESS, list.get());
			} else {
				return ResultObj.<List<NoticePersonalEx>>dataNotFound();
			}
		} catch (Exception e) {
			logger.error("normal error", e);
			return ResultObj.<List<NoticePersonalEx>>errorUnknown();
		}
	}
